...
@router.post(
    ...
    response_model=BulkApproveDuplicateResponse,
)
async def bulk_approve_duplicate_candidates(
    payload: BulkApproveDuplicateRequest,
    db: Session = Depends(get_db),
) -> BulkApproveDuplicateResponse:
    service = DedupReviewService(db)
    return service.bulk_approve_duplicates(
        reviewed_by_user_id=payload.reviewed_by_user_id,
