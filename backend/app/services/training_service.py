...
class TrainingService:
    ...
    def _level_reason(self, level_value: int) -> str:
        if level_value <= 0:
            return "该题不以这个素养为核心，不需要单独标出。"
        if level_value == 1:
            return "该素养有体现，但更多是辅助理解或局部步骤，属于弱体现。"
        if level_value == 2:
            return "该素养支撑了关键求解过程，对完成题目有明显作用，属于中等体现。"
        return "该素养决定了整题的理解或求解路径，是核心驱动力，属于强体现。"
